...
1. **Open Firewall (Port 80):** Certbot _requires_ **port 80** for its renewal challenge. We must add this `ufw` rule on our server, or the renewal will fail.
   ```bash
...
3. **Add the Renewal Job:** Add this line to the bottom of the file. It tells the server to try renewing the certificate every day at 2:30 AM.
   ...
   **Note:** The `--post-hook` is critical. It _guarantees_ AdGuard Home restarts even if the renewal fails, which prevents a service outage.
